 ...
 */
export const transformerCustomTags = () => {
    ...
    return {
        ...
        code(code) {
            const lines = code.children.filter((x) => x.type === 'element');
            const tagsToInsert = [];
            const linesToRemove = [];
            ...
            const adjustedTags = tagsToInsert.map((tag) => {
                ...
                for (const removedLine of linesToRemove) {
                    ...
                    if (removedIndex !== -1 && removedIndex <= tag.afterIndex)
                        adjustedIndex--;
                }
            ...
            for (const tag of sortedTags) {
                const currentLines = code.children.filter((x) => x.type === 'element');
                ...
                if (code.children[nextIndex]?.type === 'text' && code.children[nextIndex]?.value === '\n')
                    ...
                    code.children.splice(nextIndex, 0, tagLine, { type: 'text', value: '\n' });
            }
        },
    };
};
...
//# sourceMappingURL=transformerCustomTags.js.map