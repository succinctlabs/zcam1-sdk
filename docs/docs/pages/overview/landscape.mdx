...
C2PA is an open standard for creating a verifiable history of an image. It tracks provenance—what tool created the content, who signed it, and what edits were made—but doesn't inherently guarantee authenticity.

ZCAM uses C2PA as its container format while adding hardware-backed guarantees that transform it from a provenance system into an authenticity system. For a deeper dive into how C2PA works and how ZCAM extends it, see [C2PA](/overview/c2pa).
