...
3. **Proof Generation** (Optional): Use `@succinctlabs/react-native-zcam1/proving` to attach zero-knowledge proofs:
...
1. **Automated Verification**: When a claim enters the triage flow, the system automatically verifies all attached photos:
   - Integrate `@succinctlabs/react-native-zcam1` (or server-side equivalent) into the backend
   - For each photo, call `verifyBindings()` or `verifyProof()` to validate authenticity
