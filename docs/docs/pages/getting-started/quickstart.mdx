...
export function VerifyPhoto({ photoUri }: { photoUri: string }) {
  const handleVerify = () => {
    ...
    const proofValid = file.verifyProof(appId);
...
import { initCapture, ZCamera, CaptureInfo, VerifiableFile } from "@succinctlabs/react-native-zcam1";
...
1. **Capture**: Takes a photo, generates a hardware-backed assertion over the photo hash (App Attest on iOS; Android Key Attestation on Android), and embeds a C2PA manifest with `succinct.bindings`.
...
3. **Verify**: Recomputes the photo hash, verifies it matches the manifest, and validates the ZK proof against the pinned platform root CA.
