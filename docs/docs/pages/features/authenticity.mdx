...
An authentic photo is one that was genuinely captured by a camera—not generated, not composited, not manipulated. 
...
This means the signature isn't just "someone claims this is authentic"—it's "Apple's hardware guarantees this key exists on a genuine iPhone running this specific app."
...
The signature is bound to the exact bytes of the captured image. Change a single pixel, and verification fails.

## What Authenticity Doesn't Guarantee

Authenticity proves the photo came from a real camera—but not that the *scene* was real. Someone could photograph a screen displaying an AI-generated image. The photo would be authentically captured, but the content would be misleading.

ZCAM addresses this with [contextual metadata](/features/security#physical-replay-attacks) that helps detect physical replay attacks: depth maps, ambient light, focus distance, and more.
