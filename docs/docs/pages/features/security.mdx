  ...
  1. **Device Authenticity**: The photo was signed on a genuine device with a Secure Enclave
  2. **App Binding**: The signing key is bound to a specific application (via Apple App Attest on iOS; Android Key Attestation + Google Play Integrity on Android, planned)
  3. **Content Integrity**: The photo has not been modified since the signature was created
  4. **Proof Validity**: The above properties are verified in a zero-knowledge proof that chains to the platform root certificate
  ...
  |------------|-----------|
  | Secure Enclave keys cannot be exfiltrated | Hardware security guarantee from Apple / Android |
  | Apple App Attest is honest | Apple signs attestations only for legitimate device/app pairs |
  | Android Key Attestation + Play Integrity are honest | Google provides hardware-backed key and app integrity guarantees |
...
GPS location and timestamp embedding are **opt-in** features enabled via the [`captureLocationEnabled`](/sdk/capture/ZCamera#captureLocationEnabled) and [`captureTimestampEnabled`](/sdk/capture/ZCamera#captureTimestampEnabled) props on `ZCamera`. When enabled, the device queries GPS at capture time and embeds the coordinates and/or timestamp directly into the C2PA manifest.

Because these values come from GPS rather than the device clock or software, they are relatively hard to spoof:
- **GPS coordinates** require the device to have a genuine satellite fix or cell/Wi-Fi assisted location. Spoofing GPS at the OS level typically requires a jailbroken device, which is separately detected and flagged.
- **GPS timestamp** is derived from the GPS signal itself, not the device clock, making it independent of any user-controlled time setting.

When location retrieval fails (e.g., permission denied, no GPS fix within the timeout), the manifest records `isLocationAvailable: false` and a `locationRetrievalStatus` reason, so verifiers can distinguish "location not requested" from "location attempted but unavailable."
