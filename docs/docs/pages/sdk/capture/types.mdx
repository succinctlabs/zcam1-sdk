...
```tsx
type Orientation = "auto" | "portrait" | "landscape" | "landscapeLeft" | "landscapeRight"
```

- **`auto`** - Automatically determines orientation from the device accelerometer
- **`portrait`** - Force portrait orientation
- **`landscape`** - Force landscape, auto-detecting left or right from the accelerometer
- **`landscapeLeft`** - Force landscape with home button on the left
- **`landscapeRight`** - Force landscape with home button on the right
