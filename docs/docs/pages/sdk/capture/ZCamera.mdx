...
- **Type:** [`CameraFilter`](/sdk/capture/types#camerafilter) | `string`
- **Default:** `"normal"`

Filter preset to apply to preview and captured photos. Built-in options: `"normal"`, `"mellow"`, `"bw"`, `"nostalgic"`. Can also reference a custom filter name defined in `customFilters`.

### filterOverrides

- **Type:** `Partial<Record<CameraFilter, FilterRecipe>>`
- **Optional**

Override built-in filter presets with custom recipes. When a preset name is used with `filter` prop and an override exists, the custom recipe is applied instead.
...
- **Type:** `Record<string, FilterRecipe>`
- **Optional**

Define additional custom filters by name. Reference them using the `filter` prop.

```tsx
<ZCamera
  filter={"myVintage" as CameraFilter}
  customFilters={{
    myVintage: [
...
See [`FilterEffect`](/sdk/capture/types#filtereffect) for all available effect types.
