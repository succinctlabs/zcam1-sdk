...
The Picker SDK (`@succinctlabs/react-native-zcam1`) provides an image picker component with built-in authenticity status badges.
...
- `onSelect` - Callback when an image is tapped (single-select mode)
- `renderBadge` - (Optional) Custom badge renderer `(uri: string, status: AuthenticityStatus) => ReactElement | null`
- `refreshToken` - (Optional) Change to force reload images
- `renderFromBottom` - (Optional) Start rendering from bottom of list
- `sortOrder` - (Optional) `"newest-first"` or `"oldest-first"` (default: `"oldest-first"`)
- `multiSelect` - (Optional) Enable multi-selection mode
- `selectedUris` - (Optional) Array of selected URIs (multi-select mode)
- `onSelectionChange` - (Optional) Callback when selection changes `(uris: string[]) => void`
- `renderSelectionOverlay` - (Optional) Custom selection overlay renderer
...
```tsx
import { AuthenticityStatus } from "@succinctlabs/react-native-zcam1";
...
import { ZImagePicker, AuthenticityStatus, privateDirectory } from "@succinctlabs/react-native-zcam1";
...
function renderBadge(uri: string, status: AuthenticityStatus): React.ReactElement | null {
  switch (status) {
