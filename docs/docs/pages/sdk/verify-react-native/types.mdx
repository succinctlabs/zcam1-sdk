...
- **`isLocationAvailable`** — `true` if location access was granted and coordinates were successfully retrieved at capture time. `false` if location was attempted but could not be obtained. Absent if location access wasn't requested.
...
- [`PhotoMetadataInfo.location`](#photometadatainfo) - Location where the photo was captured
- [`VideoMetadataInfo.location`](#videometadatainfo) - Location where the video was recorded
