...
[`CaptureMetadata`](/sdk/verify/types#capturemetadata) `| null` - The capture metadata object, or `null` if not present in the manifest.
...
import { VerifiableFile, type CaptureMetadata } from '@succinctlabs/react-native-zcam1-verify';
...
- If a non-default [film style](/sdk/capture/types#camerafilmstyle) was applied at capture time, `parameters.filmStyle` contains the style name, source, and full recipe (see [`FilmStyleInfo`](/sdk/verify/types#filmstyleinfo))
