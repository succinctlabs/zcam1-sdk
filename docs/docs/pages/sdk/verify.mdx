      ...
      {result && (
        <View style={styles.result}>
          <Text style={styles.title}>
            {result.proofValid ? "✓ Authentic" : "✗ Not Authentic"}
          </Text>
...
export function verifyPhoto(uri: string, appId: string): VerificationStatus {
  try {
    const file = new VerifiableFile(uri);

    if (!file.verifyProof(appId)) {
      return { status: "invalid_proof" };
    }

    return { status: "authentic" };
  } catch (error: any) {
    if (error.message?.includes("no manifest") || error.message?.includes("not found")) {
