...
async fn proof_generation_test() {
    let app = build_app();
    ...
    let request_id = response.text();
    let mut response;

    loop {
        response = server.get(&format!("/ios/proof/{request_id}")).await;

        if response.status_code() != StatusCode::ACCEPTED {
            break;
        }
