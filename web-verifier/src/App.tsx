  ...
  const handleVerify = async () => {
    if (verifiableFile) {
      switch (fileStatus) {
        ...
        case AuthenticityStatus.Proof: {
          const isValid = await verifiableFile.verifyProof(appId);
          setIsValid(isValid.unwrapOr(false));
        ...
        {(bindings || proof) && (
          <div>
            ...
            <ul>
              ...
              {proof && (
                <li>
                  The <strong>App ID</strong> identifies the iOS application
                  that captured the asset. It is used to verify that the proof
                  was generated by a known and trusted app.
                </li>
              )}
            </ul>
            {proof && (
