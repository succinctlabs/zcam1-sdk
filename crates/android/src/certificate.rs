...
pub fn decode_certificate_chain(attestation: &str) -> Result<Vec<Certificate>, Error> {
    // Android's Base64.DEFAULT inserts newlines every 76 chars — strip them.
    let clean_attestation: String = attestation.chars().filter(|c| !c.is_whitespace()).collect();

    let outer_decoded = Base64::decode_vec(&clean_attestation)
    ...
    for (i, cert_b64) in cert_strings.iter().enumerate() {
        // Also strip whitespace from individual cert base64 strings
        let clean_cert: String = cert_b64.chars().filter(|c| !c.is_whitespace()).collect();
        let cert_der = Base64::decode_vec(&clean_cert)
