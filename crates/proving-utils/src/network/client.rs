...
use crate::network::{
    ...
    proto::{
        ...
        artifact::{
            ArtifactType, CreateArtifactRequest, artifact_store_client::ArtifactStoreClient,
        },
        ...
        auction_types::{
            ...
            GetNonceRequest as AuctionGetNonceRequest,
            GetProgramRequest as AuctionGetProgramRequest,
            ...
            MessageFormat as AuctionMessageFormat,
            RequestProofRequest as AuctionRequestProofRequest,
        ...
        types::{
            ...
            FulfillmentStatus, FulfillmentStrategy, GetProofRequestDetailsRequest,
            GetProofRequestDetailsResponse, MessageFormat, ProofMode,
        },
    },
    retry::{self, DEFAULT_RETRY_TIMEOUT, RetryableRpc},
    signer::NetworkSigner,
    utils::{Signable, sign_message},
};
...
impl RetryableRpc for NetworkClient {
    ...
    where
        ...
        T: Send,
    {
        self.with_retry_timeout(operation, DEFAULT_RETRY_TIMEOUT, operation_name)
            .await
    }
...
impl NetworkClient {
            ...
            NetworkMode::Mainnet => {
                self.with_retry(
                    || async {
                        let mut rpc = self.auction_prover_network_client().await?;
                        match rpc
                            .get_program(AuctionGetProgramRequest {
            ...
            NetworkMode::Reserved => {
                self.with_retry(
                    || async {
                        let mut rpc = self.base_prover_network_client().await?;
                        match rpc
                            .get_program(BaseGetProgramRequest {
                                vk_hash: vk_hash.to_vec(),
                            })
                            .await
                        {
    ...
    pub async fn create_program(
        ...
        let program_uri = self
            .create_artifact_with_content(&mut store, ArtifactType::Program, &elf)
            .await?;
    ...
    pub async fn get_proof_request_status<P: DeserializeOwned>(
        ...
        let proof = match status {
            FulfillmentStatus::Fulfilled => {
                let proof_uri = res.proof_uri().unwrap();
        ...
        self.with_retry(
            || async {
                let response = self
                    ...
                    .send()
                    .await
                    .context("Failed to download from URI")?;
