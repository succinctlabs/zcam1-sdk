...
/// Generate a fresh cert chain and return the full PEM string.
fn generate_fresh_cert_chain() -> String {
    ...
    zcam1_common::generate_cert_chain(jwk, "Test Subject", "Test Org")
        .expect("generate_cert_chain should succeed")
}

/// Extract the leaf (first) certificate DER bytes from a PEM chain.
fn leaf_der_from_chain(chain_pem: &str) -> Vec<u8> {
...
#[test]
fn test_leaf_cert_c2pa_profile() {
    let chain = generate_fresh_cert_chain();
    let der = leaf_der_from_chain(&chain);
...
#[test]
fn test_generate_cert_chain_produces_valid_pem() {
    use base64ct::{Base64UrlUnpadded, Encoding};
    use p256::ecdsa::SigningKey;
    use p256::elliptic_curve::rand_core::OsRng;
    ...
    let cert_chain = zcam1_common::generate_cert_chain(jwk, "Test Subject", "Test Org")
        .expect("generate_cert_chain should succeed");

    // Should contain exactly 3 PEM certificate blocks (leaf, intermediate, root)
    let cert_count = cert_chain.matches("-----BEGIN CERTIFICATE-----").count();
    assert_eq!(cert_count, 3, "Certificate chain should contain exactly 3 certificates");

    // Each block should be parseable
    let pems: Vec<_> = Pem::iter_from_buffer(cert_chain.as_bytes())
        .collect::<Result<Vec<_>, _>>()
        .expect("All PEM blocks should parse");
    assert_eq!(pems.len(), 3);

    // Each PEM should parse as a valid X.509 certificate
    for (i, pem) in pems.iter().enumerate() {
        let (_, cert) = X509Certificate::from_der(&pem.contents)
            .unwrap_or_else(|e| panic!("Certificate {i} should parse as X.509: {e}"));

        // Root and intermediate should be CAs, leaf should not
        if i == 0 {
            assert!(!cert.tbs_certificate.is_ca(), "Leaf cert (index 0) should not be a CA");
        } else {
            assert!(cert.tbs_certificate.is_ca(), "Certificate {i} should be a CA");
        }
    }
}

#[test]
fn test_verifier_trait_with_real_attestation() {
    use zcam1_common::Verifier;
    use zcam1_ios::{IosRegisterInputs, IosVerifier};

    let verifier = IosVerifier;
    let inputs = IosRegisterInputs {
